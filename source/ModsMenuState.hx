...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		noModsTxt = new FlxText(0, 0, FlxG.width, "No hay Mods Instalados\nPresiona Regresar Para Instalar Uno", 48);
		if(FlxG.random.bool(0.1)) noModsTxt.text += '\nBITCH.'; //meanie
		noModsTxt.setFormat(Paths.font("Krabby Patty.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		buttonToggle.label.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.WHITE, CENTER);
		...
		buttonUp.label.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonDown.label.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonTop.label.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		startX -= 190;
		buttonDisableAll = new FlxButton(startX, 0, "Desabilitar Todos", function() {
			for (i in modsList)
			{
				i[1] = false;
			}
		...
		buttonDisableAll.label.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		startX -= 190;
		buttonEnableAll = new FlxButton(startX, 0, "Habilitar Todos", function() {
			for (i in modsList)
			{
				i[1] = true;
			}
		...
		buttonEnableAll.label.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		installButton.label.setFormat(Paths.font("Krabby Patty.ttf"), 16, FlxColor.WHITE, CENTER);
		...
		removeButton.label.setFormat(Paths.font("Krabby Patty.ttf"), 16, FlxColor.WHITE, CENTER);
		...
		descriptionTxt.setFormat(Paths.font("Krabby Patty.ttf"), 32, FlxColor.WHITE, LEFT);
		...
		for (mod in mods)
		{
			...
			if(i == curSelected)
			{
				...
				if (mod.restart){//finna make it to where if nothing changed then it won't reset
					descriptionTxt.text += " (Este Mod Reiniciara El Juego!)";
				}
...
class ModMetadata
{
	...
	public function new(folder:String)
	{
		this.folder = folder;
		this.name = folder;
		this.description = "No Hay descripcion.";
