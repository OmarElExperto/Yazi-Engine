...
class WeekEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		txtWeekTitle.setFormat("Krabby Patty", 32, FlxColor.WHITE, RIGHT);
		...
		missingFileText.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		txtTracklist.font = Paths.font("Krabby Patty.ttf");
	...
	function addEditorBox() {
		...
		var saveWeekButton:FlxButton = new FlxButton(0, 650, "Salvar", function() {
			saveWeek(weekFile);
		});
	...
	function addWeekUI() {
		...
		hideCheckbox = new FlxUICheckBox(10, weekFileInputText.y + 40, null, null, "Ocultar Week Del Story Mode?", 100);
		...
		tab_group.add(new FlxText(backgroundInputText.x, backgroundInputText.y - 18, 0, 'Fondo:'));
		tab_group.add(new FlxText(displayNameInputText.x, displayNameInputText.y - 18, 0, 'Nombre a Mostrar:'));
		tab_group.add(new FlxText(weekNameInputText.x, weekNameInputText.y - 18, 0, 'Nombre de Week (Para el Menu de Reseteo):'));
		tab_group.add(new FlxText(weekFileInputText.x, weekFileInputText.y - 18, 0, 'Archivo de Week:'));
	...
	function addOtherUI() {
		...
		lockedCheckbox = new FlxUICheckBox(10, 30, null, null, "La Week Empieza Bloqueada?", 100);
		...
		hiddenUntilUnlockCheckbox = new FlxUICheckBox(10, lockedCheckbox.y + 25, null, null, "Ocultar Mientras Esta Bloqueada", 110);
		...
		tab_group.add(new FlxText(weekBeforeInputText.x, weekBeforeInputText.y - 28, 0, 'Nombre del Archivo de la Week a Pasar\nPara Desbloquear'));
		tab_group.add(new FlxText(difficultiesInputText.x, difficultiesInputText.y - 20, 0, 'Dificultades:'));
		tab_group.add(new FlxText(difficultiesInputText.x, difficultiesInputText.y + 20, 0, 'Dificultades Predetermiandas Son: \n"Easy, Normal, Hard"\nSin las Comillas.'));
	...
	function reloadWeekThing() {
		...
		if(isMissing) {
			...
			missingFileText.text = 'Archivo Faltante: images/storymenu/' + assetName + '.png';
		}
		recalculateStuffPosition();

		#if desktop
		// Updating Discord Rich Presence
		DiscordClient.changePresence("Editor de Weeks", "Editando: " + weekFileName);
		#end
	}
	...
	private static function onLoadComplete(_):Void
	{
		...
		_file = null;
		#else
		trace("El archivo no pudo ser cargado, No esta en el escritorio, Eres tu?");
		#end
	}
		...
		private static function onLoadCancel(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file = null;
		trace("Carga de archivos Cancelada.");
	}
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	function addEditorBox() {
		...
		var saveWeekButton:FlxButton = new FlxButton(0, 685, "Salvar", function() {
	...
	function addFreeplayUI() {
		...
		var hideFreeplayCheckbox:FlxUICheckBox = new FlxUICheckBox(10, iconInputText.y + 30, null, null, "Ocultar Week En EL Freeplay?", 100);
		...
		tab_group.add(new FlxText(10, bgColorStepperR.y - 18, 0, 'Seleccionar Color de Fondo R/G/B:'));
		tab_group.add(new FlxText(10, iconInputText.y - 18, 0, 'Seleccionar Icono:'));
