...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		DiscordClient.changePresence("Editor de Personaje de Menu", "Editando: " + characterFile.image);
		#end
		...
		txtOffsets.setFormat("Krabby Patty", 32, FlxColor.WHITE, CENTER);
		txtOffsets.alpha = 0.7;
		add(txtOffsets);

		var tipText:FlxText = new FlxText(0, 540, FlxG.width,
			"Flechitas - Cambiar Offset (Presiona shift para mover 10x mas Rapido)
			\nEspacio - Reproducir Animacion \"Start Press\" (Tipo de Personaje Boyfriend)", 16);
		tipText.setFormat(Paths.font("Krabby Patty.ttf"), 16, FlxColor.WHITE, CENTER);
	...
	function addEditorBox() {
		...
		var loadButton:FlxButton = new FlxButton(0, 480, "Cargar", function() {
			loadCharacter();
		});
		...
		var saveButton:FlxButton = new FlxButton(0, 480, "Salvar", function() {
			saveCharacter();
		});
	...
	function addCharacterUI() {
		...
		flipXCheckbox = new FlxUICheckBox(10, confirmInputText.y + 30, null, null, "Voltear", 100);
		...
		var reloadImageButton:FlxButton = new FlxButton(140, confirmInputText.y + 30, "Cargar", function() {
			reloadSelectedCharacter();
		});
		
		scaleStepper = new FlxUINumericStepper(140, imageInputText.y, 0.05, 1, 0.1, 30, 2);

		var confirmDescText = new FlxText(10, confirmInputText.y - 18, 0, 'Animacion de Start Press .XML:');
		tab_group.add(new FlxText(10, imageInputText.y - 18, 0, 'Nombre del Archivo'));
		tab_group.add(new FlxText(10, idleInputText.y - 18, 0, 'Animacion IDLE en el .XML:'));
	...
	function reloadSelectedCharacter() {
		...
		#if desktop
		// Updating Discord Rich Presence
		DiscordClient.changePresence("Editor de Personajes del Menu", "Editando: " + characterFile.image);
		#end
	}
	...
	function onLoadComplete(_):Void
	{
		...
		_file = null;
		#else
		trace("El archivo no pudo ser cargado, estasn en el Escritorio, Eres tu?");
		#end
	}
	...
	function onSaveComplete(_):Void
	{
		...
		_file = null;
		FlxG.log.notice("Archivo Cargado Exitosamente.");
	}
