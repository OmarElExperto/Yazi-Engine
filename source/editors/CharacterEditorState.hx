...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		textAnim.setFormat(Paths.font("Krabby Patty.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var tipTextArray:Array<String> = "E/Q - Camara Zoom Acercar/Alejar
		\nR - Resetear El Zoom de Camara
		\nJKLI - Mover Camara
		\nW/S - Animacion Previa/Siguiente
		\nEspacio - Reproducir Animacion
		\nFlechas Direccionales - Mover Personaje
		\nT - Resetear el Offset Predeterminado
		\nShift Para moverlo 10 Mas Rapido\n".split('\n');
	...
	function addSettingsUI() {
		...
		var reloadCharacter:FlxButton = new FlxButton(140, 20, "Cargar", function()
		{
			loadChar(!check_player.checked);
			reloadCharacterDropDown();
		});

		var templateCharacter:FlxButton = new FlxButton(140, 50, "Plantilla", function()
		{
	...
	function addCharacterUI() {
		...
		var reloadImage:FlxButton = new FlxButton(imageInputText.x + 210, imageInputText.y - 3, "Cargar Imagen", function()
		{
		...
		var decideIconColor:FlxButton = new FlxButton(reloadImage.x, reloadImage.y + 30, "Color de Icono", function()
			{
		...
		flipXCheckBox = new FlxUICheckBox(singDurationStepper.x + 80, singDurationStepper.y, null, null, "Voltear", 50);
		...
		var saveCharacterButton:FlxButton = new FlxButton(reloadImage.x, noAntialiasingCheckBox.y + 40, "Guardar", function() {
			saveCharacter();
		});
		...
		tab_group.add(new FlxText(15, imageInputText.y - 18, 0, 'Nombre de Imagen:'));
		tab_group.add(new FlxText(15, healthIconInputText.y - 18, 0, 'Nombre de Icono:'));
		tab_group.add(new FlxText(15, singDurationStepper.y - 18, 0, 'Duracion de Animacion:'));
		tab_group.add(new FlxText(15, scaleStepper.y - 18, 0, 'Escala:'));
		tab_group.add(new FlxText(positionXStepper.x, positionXStepper.y - 18, 0, 'Personaje X/Y:'));
		tab_group.add(new FlxText(positionCameraXStepper.x, positionCameraXStepper.y - 18, 0, 'Camara X/Y:'));
		tab_group.add(new FlxText(healthColorStepperR.x, healthColorStepperR.y - 18, 0, 'Color de Barra R/G/B:'));
	...
	function addAnimationsUI() {
		...
		animationLoopCheckBox = new FlxUICheckBox(animationNameInputText.x + 170, animationNameInputText.y - 1, null, null, "Deberia Repetirse?", 100);
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Añadir", function() {
			...
			genBoyOffsets();
			trace('Añadir Animacion Actual: ' + animationInputText.text);
		});

		var removeButton:FlxButton = new FlxButton(180, animationIndicesInputText.y + 30, "Eliminar", function() {
			for (anim in char.animationsArray) {
				if(animationInputText.text == anim.anim) {
					...
					reloadAnimationDropDown();
					genBoyOffsets();
					trace('Animacion Removida: ' + animationInputText.text);
					break;
				}
			}
		});
		...
		tab_group.add(new FlxText(ghostDropDown.x, ghostDropDown.y - 18, 0, 'Animacion Fantasma:'));
		tab_group.add(new FlxText(animationInputText.x, animationInputText.y - 18, 0, 'Nombre de Animacion:'));
		tab_group.add(new FlxText(animationNameFramerate.x, animationNameFramerate.y - 18, 0, 'FPS:'));
		tab_group.add(new FlxText(animationNameInputText.x, animationNameInputText.y - 18, 0, 'Animacion el El XML/.TXT:'));
		tab_group.add(new FlxText(animationIndicesInputText.x, animationIndicesInputText.y - 18, 0, 'Avanzado-Indices de Animacion:'));
	...
	function genBoyOffsets():Void
	{
		...
		if(dumbTexts.length < 1) {
			var text:FlxText = new FlxText(10, 38, 0, "Error! Animacion No Encontrada", 15);
			text.scrollFactor.set();
			text.borderSize = 1;
			dumbTexts.add(text);
	...
	function reloadAnimationDropDown() {
		...
		if(anims.length < 1) anims.push('Sin Animaciones'); //Prevents crash
	...
	function updatePresence() {
		...
		DiscordClient.changePresence("Editor de Personajes", "Personaje: " + daAnim, leHealthIcon.getCharacter());
		#end
	}
	...
	function onSaveComplete(_):Void
	{
		...
		_file = null;
		FlxG.log.notice("Archivo Guardado Exitosamente.");
	}
	...
	function onSaveError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
		FlxG.log.error("Problemas al guardar el archivo");
	}
