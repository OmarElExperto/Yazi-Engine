...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		var addLineText:FlxText = new FlxText(10, 10, FlxG.width - 20, 'Presiona O Para Remover la linea actual, Presiona P para añadir una linea despues de la actual.', 8);
		addLineText.setFormat(Paths.font("Krabby Patty.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		addLineText.scrollFactor.set();
		add(addLineText);

		selectedText = new FlxText(10, 32, FlxG.width - 20, '', 8);
		selectedText.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function addEditorBox() {
		var tabs = [
			{name: 'Linea de Dialogo', label: 'Linea de Dialogo'},
		];
	...
	function addDialogueLineUI() {
		...
		tab_group.name = "Linea de Dialogo";
		...
		angryCheckbox = new FlxUICheckBox(speedStepper.x + 120, speedStepper.y, null, null, "TextBox Agresivo", 200);
		...
		var loadButton:FlxButton = new FlxButton(20, lineInputText.y + 25, "Cargar Dialogo", function() {
			loadDialogue();
		});
		var saveButton:FlxButton = new FlxButton(loadButton.x + 120, loadButton.y, "Salvar Dialgoo", function() {
			saveDialogue();
		});

		tab_group.add(new FlxText(10, speedStepper.y - 18, 0, 'Intervalo/Velocidad (ms):'));
		tab_group.add(new FlxText(10, characterInputText.y - 18, 0, 'Personaje:'));
		tab_group.add(new FlxText(10, soundInputText.y - 18, 0, 'Nombre de Sonido:'));
	...
	function reloadCharacter() {
		...
		if(character.animation.curAnim != null && character.jsonFile.animations != null) {
			...
			animText.text = 'ERROR! ANIMACIONES NO ENCONTRADAS';
		}
	}

	private static var DEFAULT_TEXT:String = "Yazi Engine Epicardo";
	...
	function reloadText(skipDialogue:Bool) {
		...
		if(rpcText == null || rpcText.length < 1) rpcText = '(Vacio)';
		if(rpcText.length < 3) rpcText += '   '; //Fixes a bug on RPC that triggers an error when the text is too short
		DiscordClient.changePresence("Editor Epico de Dialogos", rpcText);
		#end
	}
		...
		if(!blockInput) {
			...
			for (i in 0...controlAnim.length) {
				if(controlAnim[i] && character.jsonFile.animations.length > 0) {
					...
					animText.text = 'Animacion: ' + animToPlay + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Presiona W o S para Scroll';
				}
				if(controlText[i]) {
	...
	function changeText(add:Int = 0) {
		...
		if(leLength > 0) {
			...
			character.playAnim(character.jsonFile.animations[curAnim].anim, daText.finishedText);
			animText.text = 'Animacion: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + leLength + ') - Presiona W o s Para Scroll';
		} else {
			animText.text = 'ERROR! ANIMACIONES NO ENCONTRADAS';
		}
		characterAnimSpeed();

		selectedText.text = 'Line: (' + (curSelected + 1) + ' / ' + dialogueFile.dialogue.length + ') - Presiona A o D para Scroll';
	}
	...
	function onLoadComplete(_):Void
	{
		...
		_file = null;
		#else
		trace("El archivo No Se Pudo Cargar, No esta en el Escritorio ; Eres tu?");
		#end
	}
	...
	function onLoadError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file = null;
		trace("Problemas al cargar el archivo");
	}
	...
	function onSaveError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
		FlxG.log.error("Problemas Guardando Archivo");
	}
}