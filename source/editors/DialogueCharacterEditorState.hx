...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	'JKLI - Mover camara (Shift Para Moverlo 4x mas Rapido)
	\nQ/E - Zoom Alejar/Acercar
	\nR - Resetear Camara
	\nH - Activar Fantasmas
	\nWASD - Mover Animacion Loop (Rojo)
	\nFlechitas - Mover Idle/Animacion (Azul)
	\nPresiona Shift Para Moverlos 10x mas Rapido';
	...
	override function create() {
		...
		tipText.setFormat(Paths.font("Krabby Patty.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipText.cameras = [camHUD];
		tipText.scrollFactor.set();
		add(tipText);

		offsetLoopText = new FlxText(10, 10, 0, '', 32);
		offsetLoopText.setFormat(Paths.font("Krabby Patty.ttf"), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		offsetIdleText.setFormat(Paths.font("Krabby Patty.ttf"), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		animText.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function addAnimationsUI() {
		...
		var addUpdateButton:FlxButton = new FlxButton(10, idleInputText.y + 30, "Añadir", function() {
		...
		tab_group.add(new FlxText(loopInputText.x, loopInputText.y - 18, 0, 'Nombre Loop en el .XML:'));
		tab_group.add(new FlxText(idleInputText.x, idleInputText.y - 18, 0, 'Idle/Finalizado Nombre en el .XML:'));
	...
	function addCharacterUI() {
		...
		tab_group.add(new FlxText(10, imageInputText.y - 18, 0, 'Nombre de Imagen:'));
		tab_group.add(new FlxText(10, xStepper.y - 18, 0, 'Offset:'));
		...
		var reloadImageButton:FlxButton = new FlxButton(10, scaleStepper.y + 60, "Cargar Imagen", function() {
			reloadCharacter();
		});
		
		var loadButton:FlxButton = new FlxButton(reloadImageButton.x + 100, reloadImageButton.y, "Cargar", function() {
			loadCharacter();
		});
		var saveButton:FlxButton = new FlxButton(loadButton.x, reloadImageButton.y - 25, "Salvar", function() {
			saveCharacter();
		});
	...
	private static var DEFAULT_TEXT:String = 'Yazi Engine Por Omar El Experto, Empoderado por Psych Engine !!!!El Primer FNF En Español!!!';
	...
	function reloadCharacter() {
		...
		curAnim = 0;
		animText.text = 'Animacion: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Presiona W or S para Scroll';

		#if desktop
		// Updating Discord Rich Presence
		DiscordClient.changePresence("Editor de Personaje de Dialogo", "Editando: " + character.jsonFile.image);
		#end
	}
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(UI_mainbox.selected_tab_id != lastTab) {
				if(UI_mainbox.selected_tab_id == 'Animaciones') {
					...
					character.playAnim(character.jsonFile.animations[curAnim].anim);
					animText.text = 'Animacion: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Press W or S to scroll';
				}
				lastTab = UI_mainbox.selected_tab_id;
				currentGhosts = 0;
			}
			
			if(UI_mainbox.selected_tab_id == 'Personajes')
			{
	...
	function onLoadComplete(_):Void
	{
		...
		if(fullPath != null) {
			...
			if(rawJson != null) {
				...
				if(loadedChar.dialogue_pos != null) //Make sure it's really a dialogue character
				{
					...
					trace("Archivo Cargado Exitosamente: " + cutName);
		...
		_file = null;
		#else
		trace("El Archivo no se Pudo Cargar! No estas en el Escritorio, Eres tu?");
		#end
	}
	...
	function onLoadCancel(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file = null;
		trace("Carga de Archivo Cancelada.");
	}
	...
	function onLoadError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file = null;
		trace("Problema Cargando El Archivo");
	}
	...
	function onSaveComplete(_):Void
	{
		...
		_file = null;
		FlxG.log.notice("Archivo Guardad Exitosamente");
	}
	...
	function onSaveError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
		FlxG.log.error("Problemas Guardando El Archivo");
	}
