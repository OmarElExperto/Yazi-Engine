...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Hey!', "Reproduce la animacion \"Hey!\"De Bopeebo,\nValor 1: BF = Solo Boyfriend, GF = Solo Girlfriend,\n O algo asi = Ambos.\nValor 2: Duracion de la Animacion,\ndejar en blanco para 1.7s"],
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Editor de Charteo", StringTools.replace(_song.song, '-', ' '));
		#end
		...
		var tabs = [
			{name: "Cancion", label: 'Cancion'},
			{name: "Seccion", label: 'Seccion'},
		...
		text =
		"W/S o Rueda del Mouse - Cambia el strum del Conductor
		\nA/D - Va a la Seccion Antes / Despues Del Editor
		\nZurda/Derecha - Cambiar Snap
		\nUp/Down - Cambia el strum del Conductor Mientras Hace Snap
		\nBracket Zurdo/Bracket Derecho -Cambia El Rango de Tiempo (SHIFT para mas velocidad)
		\nALT + Bracket Zurdo / Bracket Derecho - Resetea El Rango de Tiempo
		\nAgarra Shift Para Mover mas rapido
		\nAgarra Control y Clickea la nota para Seleccionarla
		\nZ/X - Zoom Acercar/Alejar
		\n
		\nEsc - Probar Charteo Adentro del Editor de Charteo
		\nEnter - Jugar tu charteo
		\nQ/E - Disminuir/Incrementar lo largo de la Nota
		\nSpace - Parar/Reanudar la cancion";

		var tipTextArray:Array<String> = text.split('\n');
		for (i in 0...tipTextArray.length) {
			var tipText:FlxText = new FlxText(UI_box.x, UI_box.y + UI_box.height + 8, 0, tipTextArray[i], 16);
			tipText.y += i * 12;
			tipText.setFormat(Paths.font("Krabby Patty.ttf"), 14, FlxColor.WHITE, LEFT/*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
	...
	function addSongUI():Void
	{
		...
		var check_voices = new FlxUICheckBox(10, 25, null, null, "Tiene Voces?", 100);
		...
		var reloadSong:FlxButton = new FlxButton(saveButton.x + 90, saveButton.y, "Cargar Audio", function()
		{
		...
		var reloadSongJson:FlxButton = new FlxButton(reloadSong.x, saveButton.y + 30, "Cargar JSON", function()
		{
			openSubState(new Prompt('Esta accion limpiara el progreso.\n\nProceder?', 0, function(){loadJson(_song.song.toLowerCase()); }, null,ignoreWarnings));
		});

		var loadAutosaveBtn:FlxButton = new FlxButton(reloadSongJson.x, reloadSongJson.y + 30, 'Autoguardado', function()
		{
			PlayState.SONG = Song.parseJSONshit(FlxG.save.data.autosave);
			MusicBeatState.resetState();
		});

		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Cargar Eventos', function()
		{
		...
		var saveEvents:FlxButton = new FlxButton(110, reloadSongJson.y, 'Salvar Evento', function ()
		{
			saveEvents();
		});

		var clear_events:FlxButton = new FlxButton(320, 310, 'Limpiar Eventos', function()
			{
		...
		var clear_notes:FlxButton = new FlxButton(320, clear_events.y + 30, 'Limpiar Notas', function()
			{
		...
		tab_group_song.add(new FlxText(stageDropDown.x, stageDropDown.y - 15, 0, 'Escenario:'));
		tab_group_song.add(new FlxText(noteSkinInputText.x, noteSkinInputText.y - 15, 0, 'Textura de Nota:'));
		tab_group_song.add(new FlxText(noteSplashesInputText.x, noteSplashesInputText.y - 15, 0, 'Textura de Splash de Nota:'));
	...
	function addSectionUI():Void
	{
		...
		tab_group_section.name = 'Seccion';

		check_mustHitSection = new FlxUICheckBox(10, 15, null, null, "Seccion de Presionar?", 100);
		check_mustHitSection.name = 'check_mustHit';
		check_mustHitSection.checked = _song.notes[curSec].mustHitSection;

		check_gfSection = new FlxUICheckBox(10, check_mustHitSection.y + 22, null, null, "Seccion de GF", 100);
		check_gfSection.name = 'check_gf';
		check_gfSection.checked = _song.notes[curSec].gfSection;
		// _song.needsVoices = check_mustHit.checked;

		check_altAnim = new FlxUICheckBox(check_gfSection.x + 120, check_gfSection.y, null, null, "Animacion Alt", 100);
		...
		var copyButton:FlxButton = new FlxButton(10, 190, "Copiar Seccion", function()
		{
			notesCopied = [];
		...
		var pasteButton:FlxButton = new FlxButton(copyButton.x + 100, copyButton.y, "Pegar Seccion", function()
		{
		...
		var clearSectionButton:FlxButton = new FlxButton(pasteButton.x + 100, pasteButton.y, "Limpiar", function()
		{
			if(check_notesSec.checked)
			{
		...
		var swapSection:FlxButton = new FlxButton(12, check_notesSec.y + 30, "Swapear", function()
		{
			for (i in 0..._song.notes[curSec].sectionNotes.length)
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
		...
		var copyLastButton:FlxButton = new FlxButton(10, swapSection.y + 30, "Copiar Ultima Seccion", function()
		{
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Opuesto", function()
		{
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Notas Espejo", function()
		{
			var duetNotes:Array<Array<Dynamic>> = [];
			for (note in _song.notes[curSec].sectionNotes)
			{
		...
		tab_group_section.add(new FlxText(stepperBeats.x, stepperBeats.y - 15, 0, 'Beats por Seccion:'));
	...
	function addNoteUI():Void
	{
		...
		tab_group_note.add(new FlxText(10, 10, 0, 'Largo de Nota:'));
		tab_group_note.add(new FlxText(10, 50, 0, 'Tiempo de Strum (en Milisegundos):'));
	...
	function addEventsUI():Void
	{
		...
		var text:FlxText = new FlxText(20, 30, 0, "Eventos:");
		...
		selectedEventText = new FlxText(addButton.x - 100, addButton.y + addButton.height + 6, (moveRightButton.x - addButton.x) + 186, 'Evento Seleccionado: Nulo');
	...
	function changeEventSelected(change:Int = 0)
	{
		if(curSelectedNote != null && curSelectedNote[2] == null) //Is event note
		{
			...
			selectedEventText.text = 'Evento Seleccionado: ' + (curEventSelected + 1) + ' / ' + curSelectedNote[1].length;
		}
		else
		{
			curEventSelected = 0;
			selectedEventText.text = 'Evento Seleccionado: Nulo';
		}
		updateNoteUI();
	}
	...
	function addChartingUI() {
		...
		waveformUseInstrumental = new FlxUICheckBox(10, 90, null, null, "Onda Para Instrumental", 100);
		...
		waveformUseVoices = new FlxUICheckBox(waveformUseInstrumental.x + 120, waveformUseInstrumental.y, null, null, "Onda Para Voces", 100);
		...
		check_mute_inst = new FlxUICheckBox(10, 310, null, null, "Silenciar Instrumental (En El Editor)", 100);
		...
		mouseScrollingQuant = new FlxUICheckBox(10, 200, null, null, "Quantizacion del Scroll de Mouse", 100);
		...
		check_vortex = new FlxUICheckBox(10, 160, null, null, "Editor de Vortex (BETA)", 100);
		...
		check_warnings = new FlxUICheckBox(10, 120, null, null, "Ignorar Advertencias", 100);
		...
		var check_mute_vocals = new FlxUICheckBox(check_mute_inst.x + 120, check_mute_inst.y, null, null, "Silenciar Vocales (En El Editor)", 100);
		...
		playSoundDad = new FlxUICheckBox(check_mute_inst.x + 120, playSoundBf.y, null, null, 'Reproducir Sonido (Notas del OP)', 100,
			function() {
		...
		metronome = new FlxUICheckBox(10, 15, null, null, "Metronomo Habilitado", 100,
			function() {
		...
		disableAutoScrolling = new FlxUICheckBox(metronome.x + 120, metronome.y, null, null, "Desactivar Scroll Auto (No se Recomienda)", 120,
			function() {
		...
		tab_group_chart.add(new FlxText(instVolume.x, instVolume.y - 15, 0, 'Volumen Inst'));
		tab_group_chart.add(new FlxText(voicesVolume.x, voicesVolume.y - 15, 0, 'Volumen Voices'));
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Paths.font("Krabby Patty.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				var text:String = 'Eventos: ' + note.eventName + ' (' + Math.floor(note.strumTime) + ' ms)' + '\nValor 1: ' + note.eventVal1 + '\nValor 2: ' + note.eventVal2;
				if(note.eventLength > 1) text = note.eventLength + ' Eventos:\n' + note.eventName;

				var daText:AttachedFlxText = new AttachedFlxText(0, 0, 400, text, 12);
				daText.setFormat(Paths.font("Krabby Patty.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
