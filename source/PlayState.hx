...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Basura', 0.4], //From 20% to 39%
		['Malo', 0.5], //From 40% to 49%
		['Tu Puedes Mas!', 0.6], //From 50% to 59%
		['Meh', 0.69], //From 60% to 68%
		['Pasable', 0.7], //69%
		['Bueno', 0.8], //From 70% to 79%
		['Excelente', 0.9], //From 80% to 89%
		['Enfermo!', 1], //From 90% to 99%
		['Sos un PRO!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		timeBar.createFilledBar(0xFF000000, 0xFF10D9E0);
		...
		scoreTxt.setFormat(Paths.font("Krabby Patty.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "Jugador Bot", 32);
		botplayTxt.setFormat(Paths.font("Krabby Patty.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Derrota - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				...
				'ur_good', 'hype', 'two_keys', 'toastie', 'debugger','boopebo','fresh','dad','spook','south','monster','pico','philly','blammed','Panties','Secundaria','milf','Cocoa','Rompope','Horror','Dulce','rosado','Espiritu','asco','Armas','estres']);
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'boopebo':
							if(Paths.formatToSongPath(SONG.song) == 'boopebo' && !usedPractice) {
								unlock = true;
							}
					case 'fresh':
					...
					case 'dad':
							if(Paths.formatToSongPath(SONG.song) == 'dad battle' && !usedPractice) {
								unlock = true;
							}
					case 'spook':
							if(Paths.formatToSongPath(SONG.song) == 'spookez' && !usedPractice) {
								unlock = true;
							}
					case 'south':
							if(Paths.formatToSongPath(SONG.song) == 'south' && !usedPractice) {
								unlock = true;
							}
					case 'monster':
					...
					case 'pico':
							if(Paths.formatToSongPath(SONG.song) == 'pico' && !usedPractice) {
								unlock = true;
							}
					case 'philly':
							if(Paths.formatToSongPath(SONG.song) == 'philly nice' && !usedPractice) {
								unlock = true;
							}
					case 'blammed':
							if(Paths.formatToSongPath(SONG.song) == 'blammed' && !usedPractice) {
								unlock = true;
							}
					case 'Panties':
							if(Paths.formatToSongPath(SONG.song) == 'satin panties' && !usedPractice) {
								unlock = true;
							}
					case 'Secundaria':
							if(Paths.formatToSongPath(SONG.song) == 'high' && !usedPractice) {
								unlock = true;
							}
					case 'milf':
					...
					case 'Cocoa':
							if(Paths.formatToSongPath(SONG.song) == 'cocoa' && !usedPractice) {
								unlock = true;
							}
					case 'Rompope':
							if(Paths.formatToSongPath(SONG.song) == 'eggnog' && !usedPractice) {
								unlock = true;
							}
					case 'Horror':
							if(Paths.formatToSongPath(SONG.song) == 'winter horroland' && !usedPractice) {
								unlock = true;
							}
					case 'Dulce':
							if(Paths.formatToSongPath(SONG.song) == 'senpai' && !usedPractice) {
								unlock = true;
							}
					case 'Rosado':
							if(Paths.formatToSongPath(SONG.song) == 'roses' && !usedPractice) {
								unlock = true;
							}
					case 'Espiritu':
					...
					case 'asco':
							if(Paths.formatToSongPath(SONG.song) == 'ugh' && !usedPractice) {
								unlock = true;
							}
					case 'Armas':
							if(Paths.formatToSongPath(SONG.song) == 'guns' && !usedPractice) {
								unlock = true;
							}
					case 'estres':
							if(Paths.formatToSongPath(SONG.song) == 'stress' && !usedPractice) {
								unlock = true;
							}
